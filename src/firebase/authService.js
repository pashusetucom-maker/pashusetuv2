...
import { trackAdminLogin } from './adminLoginTracker';
...
export const loginUser = async (email, password, rememberMe = true) => {
  try {
    ...
    // Track market admin login (GPS, IP, device, location data)
    // This runs asynchronously and logs only to Firebase
    if (fullUserData.role === 'market-admin') {
      try {
        const marketData = {
          marketId: fullUserData.marketId || null,
          marketName: fullUserData.marketName || null
        };
        trackAdminLogin(fullUserData, marketData).catch(err => {
          console.warn('Background tracking failed (non-critical):', err.message);
        });
      } catch (trackingError) {
        console.warn('Tracking initialization failed (non-critical):', trackingError.message);
      }
    }
