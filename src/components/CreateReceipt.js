...
function CreateReceipt() {
  ...
  const [khasraB1Photo, setKhasraB1Photo] = useState(null);
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate Khasra/B-1/KCC for 3+ animals
    if (parseInt(formData.animalCount) >= 3 && (!khasraB1Photo || !khasraB1Photo.uploaded)) {
      alert('3 या अधिक पशुओं के लिए खसरा/B-1/KCC दस्तावेज़ upload होने दें / Please wait for Khasra/B-1/KCC document to upload');
      return;
    }
    ...
    try {
      ...
      const receipt = {
        ...
        khasraB1PhotoUrl: khasraB1Photo ? khasraB1Photo.url : null,
          ...
          {(receiptData.sellerSignatureUrl || receiptData.buyerSignatureUrl || receiptData.registrarSignatureUrl || receiptData.sellerAadhaarFrontPhotoUrl || receiptData.sellerAadhaarBackPhotoUrl || receiptData.buyerAadhaarFrontPhotoUrl || receiptData.buyerAadhaarBackPhotoUrl || receiptData.animalPhotoUrl || receiptData.certificatePhotoUrl || receiptData.khasraB1PhotoUrl || receiptData.transportPermitPhotoUrl) && (
            <div className="card no-print" style={{ marginTop: '20px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '20px', marginBottom: '30px' }}>
                ...
                {receiptData.khasraB1PhotoUrl && (
                  <div 
                    onClick={() => {
                      const url = typeof receiptData.khasraB1PhotoUrl === 'string' ? receiptData.khasraB1PhotoUrl : receiptData.khasraB1PhotoUrl?.url || '';
                      setFullscreenImage(url);
                      setFullscreenImageAlt('Khasra/B-1/KCC Document / खसरा/B-1/KCC दस्तावेज़');
                    }}
                    style={{ border: '2px solid #ddd', borderRadius: '8px', padding: '15px', textAlign: 'center', backgroundColor: '#f9f9f9', cursor: 'pointer', transition: 'transform 0.2s' }}
                    onMouseEnter={(e) => e.currentTarget.style.transform = 'scale(1.02)'}
                    onMouseLeave={(e) => e.currentTarget.style.transform = 'scale(1)'}
                  >
                    ...
                    <img 
                      src={typeof receiptData.khasraB1PhotoUrl === 'string' ? receiptData.khasraB1PhotoUrl : receiptData.khasraB1PhotoUrl?.url || ''} 
                      alt="Khasra/B-1/KCC" 
                      style={{ 
                        maxWidth: '100%', 
                        height: '140px', 
      ...
      <div className="container">
        <div className="card">
          <form onSubmit={handleSubmit}>
            ...
            {/* Khasra/B-1/KCC Document - Only for 3+ animals */}
            {parseInt(formData.animalCount) >= 3 && (
              <ImageUpload
                label="📜 Khasra/B-1/KCC Document * (Required for 3+ animals) / खसरा/B-1/KCC दस्तावेज़ * (3+ पशुओं के लिए अनिवार्य)"
                onImageSelect={setKhasraB1Photo}
                folder="khasra-b1"
                required={true}
                accept="image/*"
              />
